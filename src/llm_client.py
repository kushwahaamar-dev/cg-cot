...
def _model_to_example_dict(model_cls: type[BaseModel]) -> dict:
    """Build a concise example dict from a Pydantic model."""
    import typing
    example = {}
    for name, field_info in model_cls.model_fields.items():
        ...
        if origin is list or origin is typing.List:
            # List of BaseModel subclass → show one example item
...
def _schema_to_example(model_cls: type[BaseModel]) -> str:
    """Build a concise example JSON string from a Pydantic model."""
...
class OllamaClient:
    """Local Ollama models via REST API."""
    ...
    @retry(
        retry=retry_if_exception_type(
            (ValidationError, json.JSONDecodeError, ConnectionError, requests.RequestException)
        ),
    ...
    def _track(self, data: dict, elapsed: float) -> None:
        ...
        self.call_count += 1
        logger.debug("Ollama call #%d  %.2fs", self.call_count, elapsed)
    ...
    def usage_summary(self) -> dict:
        return {"total_tokens": self.total_tokens_used, "api_calls": self.call_count}


# ── Factory ─────────────────────────────────────────────────────────

LLMClient = OllamaClient   # type alias


def make_client(provider: str = "ollama", **kwargs) -> LLMClient:
    """Convenience factory."""
    if provider == "ollama":
