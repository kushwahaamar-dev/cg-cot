...
def _extract_math_answer(solution_text: str) -> str:
    ...
    # Handle nested braces: find the outermost \boxed{...}
...
def load_gsm8k(n: int | None = None) -> list[BenchmarkProblem]:
    ...
    for i, row in enumerate(sampled):
        ...
        # count reasoning steps for difficulty proxy
...
def load_math(n: int | None = None) -> list[BenchmarkProblem]:
    ...
    # Load all subjects from EleutherAI's version and concatenate
    _MATH_SUBJECTS = [
...
def load_folio(n: int | None = None) -> list[BenchmarkProblem]:
    ...
    for i, row in enumerate(sampled):
        ...
        label_map = {"0": "True", "1": "False", "2": "Unknown",
                     "true": "True", "false": "False",
                     "uncertain": "Unknown", "unknown": "Unknown"}
        label_text = label_map.get(label.lower(), label)
        problem_text = f"Premises:\n{premises}\n\nConclusion:\n{conclusion}\n\nIs the conclusion True, False, or Unknown given the premises?"
        problems.append(
            BenchmarkProblem(
                id=f"folio_{i:04d}",
...
LOADERS = {
    BenchmarkName.GSM8K: load_gsm8k,
    BenchmarkName.MATH: load_math,
    BenchmarkName.FOLIO: load_folio,
}


def load_all_benchmarks() -> dict[BenchmarkName, list[BenchmarkProblem]]:
    """Load every benchmark at its configured sample size."""
